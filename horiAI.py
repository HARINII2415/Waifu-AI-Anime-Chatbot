...
# ==================== 🔐 API KEY ====================
OPENROUTER_API_KEY = ""  # ⚠️ Replace safely (don’t push to GitHub)
...
def get_ai_response(prompt):
    ...
    data = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are a cute anime-style assistant. Always reply in only 5 to 8 words. Be brief, casual, and natural. Don’t use emojis. Speak like I’m your boss."
            },
...
async def speak(text, tries=3, wait=2):
    global is_speaking
    if is_speaking:
        return
    is_speaking = True
    ...
    for attempt in range(1, tries + 1):
        try:
            ...
            if attempt < tries:
                ...
                print("TTS failed after retries. Please check network.")
    try:
        os.remove(filename)
...
async def tell_about_topic(command):
    triggers = ["do you know about", "tell me about", "what do you know about"]
    for phrase in triggers:
        if phrase in command.lower():
            ...
            try:
                ...
                await speak(f"Couldn't find info about {topic}.")
            return True
    return False

async def explain_meaning(command):
    triggers = ["what do you mean by", "define", "explain"]
    for phrase in triggers:
        if phrase in command.lower():
            topic = command.lower()
            for p in triggers:
            ...
            try:
                ...
                await speak(f"No meaning found for {topic}.")
            return True
    return False
...
async def set_timer(command):
    ...
    if match:
        ...
        await speak(f"Time's up! Your {value} {unit} timer finished.")
    else:
...
class AssistantGUI:
    ...
    async def handle_command(self, command):
        ...
        if "play" in command and "spotify" in command: await play_song_on_spotify(command); return
        if "exit" in command:
